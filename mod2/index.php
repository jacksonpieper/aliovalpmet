...
class tx_templavoila_module2 extends \TYPO3\CMS\Backend\Module\BaseScriptClass
{
    ...
    public function renderModuleContent_mainView()
    {
        ...
        foreach ($dsScopes as $scopePointer) {
            ...
            switch ((string) $scopePointer) {
                case \Extension\Templavoila\Domain\Model\AbstractDataStructure::SCOPE_PAGE:
                    ...
                    $scopeIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIconForRecord('pages', []);
                    break;
                case \Extension\Templavoila\Domain\Model\AbstractDataStructure::SCOPE_FCE:
                    ...
                    $scopeIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIconForRecord('tt_content', []);
                    break;
        ...
        $this->content .= $this->doc->section(\Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('title'),
            $settings .
            $this->doc->getDynTabMenu($parts, 'TEMPLAVOILA:templateOverviewModule:' . $this->id, 0, 0, 300), 0, 1);
    }
        ...
        if (count($dsList)) {
            foreach ($dsList as $dsObj) {
                ...
                $TOcontent .= '<a href="#" onclick="' . htmlspecialchars(\TYPO3\CMS\Backend\Utility\BackendUtility::editOnClick(
                        ...
                        '&defVals[tx_templavoila_tmplobj][fileref]=' . rawurlencode($newFileRef), $this->doc->backPath)) . '">' . \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('actions-document-new') . \Extension\Templavoila\Utility\GeneralUtility::getLanguageService()->getLL('createnewto', true) . '</a>';
    ...
    public function renderDataStructureDisplay(\Extension\Templavoila\Domain\Model\AbstractDataStructure $dsObj, $scope, $toIdArray)
    {
        ...
        if ($dsObj->isFilebased()) {
            ...
            $dsIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIconForRecord('tx_templavoila_datastructure', [], ['title' => $dsObj->getKey()]);
    ...
    public function renderTODisplay($toObj, $scope, $children = 0)
    {
        ...
        $recordIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIconForRecord('tx_templavoila_tmplobj', [], ['title' => $toObj->getKey()]);
...
' . $TSrecord['config'];
    ...
    protected function getTemplateFiles()
    {
        ...
        $files = [];
        foreach ($paths as $path) {
            $files = array_merge(\TYPO3\CMS\Core\Utility\GeneralUtility::getAllFilesAndFoldersInPath([], $path . ((substr($path, -1) != '/') ? '/' : ''), 'html,htm,tmpl', 0), $files);
        }
